...
function gameLoop() {
  ...
  if (gameShowingTitleScreen) {
    if (keysDownNow.length > 0) {
      ...
      if (keysDownArray.indexOf(38) >= 0)
        gameData.snakeDirection = 0; //Up
      else if (keysDownArray.indexOf(39) >= 0)
        gameData.snakeDirection = 1; //Right
      else if (keysDownArray.indexOf(40) >= 0)
        gameData.snakeDirection = 2; //Down
  ...
  if (keysDownNow.indexOf(38) >= 0) {
    if (gameData.snakeDirection != 2) gameData.snakeDirection = 0;
  } //Up
  else if (keysDownNow.indexOf(39) >= 0) {
    if (gameData.snakeDirection != 3) gameData.snakeDirection = 1;
  } //Right
  else if (keysDownNow.indexOf(40) >= 0) {
    if (gameData.snakeDirection != 0) gameData.snakeDirection = 2;
  } //Down
  else if (keysDownNow.indexOf(37) >= 0) {
    if (gameData.snakeDirection != 1) gameData.snakeDirection = 3;
  } //Left
  ...
  gameData.snakeTail.push([
    gameData.snakeX,
    gameData.snakeY,
    gameData.snakeDirection,
  ]);

  //If the tail is too long, remove the oldest piece.
  if (gameData.snakeTail.length > gameData.snakeTailLength)
    gameData.snakeTail.shift();
  ...
  else if (gameData.snakeX >= gameData.playingFieldWidth)
    gameData.snakeX -= gameData.playingFieldWidth;
  if (gameData.snakeY < 0) gameData.snakeY += gameData.playingFieldHeight;
  else if (gameData.snakeY >= gameData.playingFieldHeight)
    gameData.snakeY -= gameData.playingFieldHeight;
  ...
  gameContext.drawImage(
    ...
    goodBlock.width,
    goodBlock.height,
    gameData.goodBlock[0] * tileWidth,
    gameData.goodBlock[1] * tileHeight + verticalOffset,
    tileWidth,
    tileHeight,
  );
  ...
  for (let i = 0, l = gameData.badBlocks.length; i < l; i++) {
    ...
    gameContext.drawImage(
      ...
      blockred.height,
      block[0] * tileWidth,
      block[1] * tileHeight + verticalOffset,
      tileWidth,
      tileHeight,
    );
  }
  ...
  for (let i = 0, l = gameData.snakeTail.length; i < l; i++) {
    ...
    gameContext.drawImage(
      body,
      snakeSpriteSize * segment[2],
      0,
      snakeSpriteSize,
      body.height,
      segment[0] * tileWidth,
      segment[1] * tileHeight + verticalOffset,
      tileWidth,
      tileHeight,
    );
  }
  ...
  gameContext.drawImage(
    head,
    snakeSpriteSize * gameData.snakeDirection,
    0,
    snakeSpriteSize,
    head.height,
    gameData.snakeX * tileWidth - (snakeHeadWidth - tileWidth) / 2,
    gameData.snakeY * tileHeight +
      verticalOffset -
      (snakeHeadHeight - tileHeight) / 2,
    snakeHeadWidth,
    snakeHeadHeight,
  );
  ...
  for (let i = 0, l = gameData.snakeTail.length; i < l; i++) {
    let segment = gameData.snakeTail[i];
    if (segment[0] == gameData.snakeX && segment[1] == gameData.snakeY) {
      gameData.gameOver();
      return;
    }
  }
  ...
  for (let i = 0, l = gameData.badBlocks.length; i < l; i++) {
    let block = gameData.badBlocks[i];
    if (block[0] == gameData.snakeX && block[1] == gameData.snakeY) {
      gameData.gameOver();
      return;
    }
  }
  ...
  if (
    gameData.goodBlock[0] == gameData.snakeX &&
    gameData.goodBlock[1] == gameData.snakeY
  ) {
    ...
    if (Math.random() < 0.33)
      gameData.badBlocks.push(gameData.getRandomUnoccupiedCoordinate());
  }
}
...
function initializeGame() {
  ...
  if (!window.CanvasRenderingContext2D) {
    document.getElementById("javascriptWarning").innerHTML =
      "Your browser does not support 2D rendering via HTML5 Canvas. Please upgrade to a modern, standards-compliant browser to play this game, such as Mozilla Firefox or Google Chrome.";
    return;
  }
  ...
  scoreBox.updateScore = function (score) {
  ...
  gameData.getRandomUnoccupiedCoordinate = function () {
    while (true) {
      ...
      if (this.goodBlock.length > 0)
        if (blockX == this.goodBlock[0] && blockY == this.goodBlock[1])
          continue;
      let reroll = false;
      for (let i = 0, l = this.badBlocks.length; i < l; i++) {
        let block = this.badBlocks[i];
        if (blockX == block[0] && blockY == block[1]) {
          reroll = true;
          break;
        }
      }
      ...
      for (let i = 0, l = this.snakeTail.length; i < l; i++) {
        ...
        if (blockX == segment[0] && blockY == segment[1]) {
          reroll = true;
          break;
        }
      }
...
function touchEvent(e) {
  ...
  for (let i = 0; i < touches.length; i++) {
    ...
    if (touchRatioX < 0.3 && touchRatioY > 0.3 && touchRatioY < 0.7) {
      //Left press
      keysDownArray.push(37);
      keysDownNow.push(37);
    } else if (touchRatioX > 0.7 && touchRatioY > 0.3 && touchRatioY < 0.7) {
      //Right press
      keysDownArray.push(39);
      keysDownNow.push(39);
    } else if (touchRatioY < 0.3 && touchRatioX > 0.3 && touchRatioX < 0.7) {
      //Up press
      keysDownArray.push(38);
      keysDownNow.push(38);
    } else if (touchRatioY > 0.7 && touchRatioX > 0.3 && touchRatioX < 0.7) {
      //Down press
  ...
  if (mobileMode) {
    ...
    if (winWidth < winHeight) {
      gameContainer.style.transform =
        "translate(-50%, -50%) scale(" + winWidth / DEFAULT_GAME_WIDTH + ")";
    } else {
      gameContainer.style.transform =
        "translate(-50%, -50%) scale(" + winHeight / DEFAULT_GAME_WIDTH + ")";
    }
...
document
  .getElementById("gameExitBtn")
  .addEventListener("click", quitAndCloseGame);
